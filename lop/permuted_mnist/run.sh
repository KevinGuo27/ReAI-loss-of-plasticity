...
#SBATCH --mem=64G
#SBATCH --job-name=run
#SBATCH --output=run_cbp_800-%A_%a.out   # %A = master job ID, %a = array index
#SBATCH --error=run_cbp_800-%A_%a.err
#SBATCH --array=0-8         # launches 9 tasks with IDs 0,1,...,8
...
CONFIG="temp_cfg/cbp/${SLURM_ARRAY_TASK_ID}.json"
